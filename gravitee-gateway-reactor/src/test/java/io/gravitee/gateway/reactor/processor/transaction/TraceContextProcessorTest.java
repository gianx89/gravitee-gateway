...
public class TraceContextProcessorTest {
    ...
    public void shouldPropagateSameTraceparent() throws InterruptedException {
        ...
        new TraceContextProcessor(true)
                .handler(context -> {
    ...
    @Test
    public void shouldNotHaveTraceparent() throws InterruptedException {
        final CountDownLatch lock = new CountDownLatch(1);

        Mockito.when(request.id()).thenReturn(UUID.toString(UUID.random()));

        new TraceContextProcessor(false) // disabled processor
                .handler(context -> {
                    Assert.assertNull(context.response().headers().getFirst(HEADER_TRACE_PARENT));
                    lock.countDown();
                }).handle(context);
